...
// Validate the input
if (!isset($input['order_id']) || !isset($input['qc_status'])) {
    echo json_encode([
        "status" => "error",
        "message" => "Missing required parameters: order_id or qc_status"
    ]);
    exit();
}

$order_id = $conn->real_escape_string($input['order_id']);
$qc_status = $conn->real_escape_string($input['qc_status']);

// Validate the status value
$valid_statuses = ['pending', 'ongoing', 'completed'];
if (!in_array($qc_status, $valid_statuses)) {
    echo json_encode([
        "status" => "error",
...
// Update the qc_status in the database
$sql_update = "UPDATE orders SET qc_status = '$qc_status' WHERE order_id = '$order_id'";

if ($conn->query($sql_update) === TRUE) {
    // Fetch the updated order details
    $sql_select = "SELECT order_id, qc_status FROM orders WHERE order_id = '$order_id'";
