...
class _PrintingState extends State<Printing> {
  ...
  Future<void> _updatePrintingStatus(String orderId, String status) async {
    ...
    try {
      final response = await http.post(
        ...
        body:
            json.encode({'order_id': orderId, 'printing_stage_status': status}),
      );
          ...
          setState(() {
            for (var order in _orders) {
              if (order['order_id'] ==
                ...
                order['printing_stage_status'] =
                    jsonResponse['updated_order']['printing_stage_status'];
                break;
              }
            }
          });
  ...
  Widget build(BuildContext context) {
    final filteredOrders = _orders
        .where((order) =>
            (_sortOption == 'All' ||
                order['printing_stage_status'] == _sortOption.toLowerCase()) &&
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _buildStatusDropdown(order['printing_status'] ?? 'pending', order),
        ],
      ),
    );
  }
          ...
          onChanged: (String? newStatus) {
            if (newStatus != null) {
              String newValue = statusMapping.entries
                  ...
                  .key;
              setState(() {
                order['printing_status'] = newValue;
              });
              _updatePrintingStatus(order['order_id'], newValue);
            }
          },
