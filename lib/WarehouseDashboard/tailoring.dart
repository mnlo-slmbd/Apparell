...
class _TailoringState extends State<Tailoring> {
  ...
  Future<void> _updateTailoringStatus(String orderId, String status) async {
    ...
    try {
      final response = await http.post(
        ...
        body: json.encode({'order_id': orderId, 'tailoring_status': status}),
      );
          ...
          setState(() {
            for (var order in _orders) {
              if (order['order_id'] ==
                ...
                order['tailoring_status'] =
                    jsonResponse['updated_order']['tailoring_status'];
                break;
              }
            }
          });
  ...
  Widget build(BuildContext context) {
    final filteredOrders = _orders
        .where((order) =>
            (_sortOption == 'All' ||
                order['tailoring_status'] == _sortOption.toLowerCase()) &&
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    _buildHeaderCell('Tailoring Status', 2),
                  ],
                ),
              ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _buildStatusDropdown(order['tailoring_status'] ?? 'Pending', order),
        ],
      ),
    );
  }
    ...
    return Expanded(
      ...
      child: Container(
        ...
        child: DropdownButton<String>(
          value: statusMapping[status] ?? "Pending",
          ...
          onChanged: (String? newStatus) {
            if (newStatus != null) {
              String newValue = statusMapping.entries
                  ...
                  .key;
              setState(() {
                order['tailoring_status'] = newValue;
              });
              _updateTailoringStatus(order['order_id'], newValue);
            }
          },
