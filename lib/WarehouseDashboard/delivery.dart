...
class _DeliveryState extends State<Delivery> {
  ...
  Future<void> _updateDeliveryStatus(String orderId, String status) async {
    ...
    try {
      final response = await http.post(
        ...
        body: json.encode({'order_id': orderId, 'delivery_status': status}),
      );
            ...
            for (var order in _orders) {
              if (order['order_id'] ==
                ...
                order['delivery_status'] =
                    jsonResponse['updated_order']['delivery_status'];
                break;
              }
            }
          });
  ...
  Widget build(BuildContext context) {
    final filteredOrders = _orders
        .where((order) =>
            (_sortOption == 'All' ||
                order['delivery_status'] == _sortOption.toLowerCase()) &&
          ...
          onChanged: (String? newStatus) {
            if (newStatus != null) {
              String newValue = statusMapping.entries
                  ...
                  .key;
              setState(() {
                order['delivery_status'] = newValue;
              });
              _updateDeliveryStatus(order['order_id'], newValue);
            }
          },
